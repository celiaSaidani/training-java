                ...
                if (count > 0) {
                    try {
                        if (reqOrder != null & reqBy != null) {
                            System.out.println("in the dashboard "+ reqOrder +" "+reqBy+ " "+request.getParameter("search").trim());
                            computer = computerService.SearchOrderBy( (page - 1) * size,
                                   size,reqOrder, reqBy,request.getParameter("search").trim());
